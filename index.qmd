...
<section id="hero-slider" style="position: relative; width: 100%; height: 55vh; min-height: 380px; max-height: 529px; overflow: hidden; background: #0d0d0d;">
  ...
  <div id="hero-content" style="position: absolute; inset: 0; z-index: 4; display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 1.5rem 1.5rem 3.5rem; text-align: center;">
    ...
    <div style="margin-bottom: 0.6rem;">
      <span id="hero-year" style="font-family: 'Inter', sans-serif; font-size: clamp(1rem, 2vw, 1.25rem); font-weight: 700; color: #C9A227; line-height: 1; display: inline-block; letter-spacing: 0.08em; padding: 0.3rem 0.9rem; border: 1px solid rgba(201,162,39,0.35); border-radius: 4px; backdrop-filter: blur(8px); background: rgba(0,0,0,0.25); transition: opacity 300ms ease;">2006</span>
    ...
    <h1 style="font-family: 'Inter', sans-serif; font-style: normal; font-size: clamp(1.4rem, 3.2vw, 2.25rem); font-weight: 700; color: #FFFFFF; letter-spacing: -0.02em; line-height: 1.25; margin-bottom: 0.5rem; max-width: 680px; text-shadow: 0 1px 8px rgba(0,0,0,0.3);">
      Excellence is not what we aspire to — <span style="color: #C9A227; font-family: 'Playfair Display', Georgia, serif; font-style: italic;">it's what we deliver.</span>
    </h1>
    ...
    <div style="display: flex; gap: 0.6rem; justify-content: center; flex-wrap: wrap;">
      <a href="about/index.html" class="hero-cta-primary" style="display: inline-flex; align-items: center; padding: 0.45rem 1.4rem; font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 600; letter-spacing: 0.03em; color: #2B2B2B; background: #C9A227; border: none; border-radius: 4px; text-decoration: none; transition: all 250ms ease; box-shadow: 0 2px 8px rgba(201,162,39,0.3);">Learn More</a>
  ...
  <div id="hero-nav-bar" style="position: absolute; bottom: 0; left: 0; right: 0; z-index: 5; display: flex; align-items: center; justify-content: center; gap: 0.75rem; padding: 0.6rem 1rem; background: rgba(0,0,0,0.45); backdrop-filter: blur(12px); border-top: 1px solid rgba(255,255,255,0.08);">
    <button id="hero-prev" aria-label="Previous" style="width: 32px; height: 32px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.15); background: transparent; color: #C9A227; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 200ms ease;">‹</button>
    ...
    <button id="hero-next" aria-label="Next" style="width: 32px; height: 32px; border-radius: 50%; border: 1px solid rgba(255,255,255,0.15); background: transparent; color: #C9A227; font-size: 1rem; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 200ms ease;">›</button>
  </div>
</section>
...
(function() {
  var data = [
    ...
    { year: '2007', caption: 'Flood relief', photos: [
      { src: 'images/project-images/biong-women-carrying-water.png', desc: 'Women carrying water during the crisis', pos: 'center 40%' },
    ...
    { year: '2008', caption: 'Education & community', photos: [
      ...
      { src: 'images/project-images/best_photos/best_pictures-07.jpg', desc: 'Intergenerational respect and cultural authenticity', pos: 'center 45%' }
    ]},
    ...
    { year: '2025', caption: '501(c)(3) registered', photos: [
      { src: 'images/yet_compassion_official_registration.png', desc: 'Y.E.T. Compassion officially registered as a 501(c)(3) nonprofit in Nashville, TN', pos: 'center center' }
    ]}
  ];
  ...
  function render() {
    ...
    // Check if this is the full-bleed registration image
    var isRegistration = p.src.indexOf('yet_compassion_official_registration') !== -1;
    var heroSection = document.getElementById('hero-slider');
    if (isRegistration) {
      contentEl.style.cssText = 'display:none !important';
      overlayEl.style.display = 'none';
      bg.style.objectFit = 'contain';
      bg.style.objectPosition = 'center center';
      bg.style.transform = 'scale(1)';
      heroSection.style.background = 'radial-gradient(ellipse at center, #122e1e 0%, #0a1f12 50%, #06140b 100%)';
    } else {
      ...
      heroSection.style.background = '#0d0d0d';
    }
    ...
    if (g.photos.length > 1) {
      g.photos.forEach(function(_, i) {
        ...
        dot.style.background = i === photoIdx ? '#C9A227' : 'rgba(255,255,255,0.25)';
        if (i === photoIdx) dot.style.boxShadow = '0 0 6px rgba(201,162,39,0.4)';
    ...
    Array.from(pipsEl.children).forEach(function(pip, i) {
      if (i === yearIdx) {
        pip.style.background = '#C9A227';
        pip.style.color = '#2B2B2B';
        pip.style.borderColor = '#C9A227';
        pip.style.boxShadow = '0 0 8px rgba(201,162,39,0.3)';
      } else {
...
<section class="reveal-on-scroll" style="background-color: #F7F6F2; padding: 3.5rem 1.5rem;">
  <div style="max-width: 960px; margin: 0 auto;">
    ...
    <div style="text-align: center; margin-bottom: 2.5rem;">
      <p style="font-family: 'Inter', sans-serif; font-size: 0.8rem; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #C9A227; margin-bottom: 0.75rem;">Brand Identity</p>
    ...
    <div class="emblem-layout" style="display: grid; grid-template-columns: 1fr 1fr; gap: 2.5rem; align-items: center;">
      ...
      <div style="text-align: center;">
        <div style="background: #0B1F3B; border-radius: 16px; padding: 2.5rem 2rem; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 20px rgba(0,0,0,0.12);">
          <img src="images/logo.png" alt="Y.E.T. Compassion emblem — golden cross within a blue heart on a deep navy circle" style="max-width: 180px; width: 100%; height: auto;">
        </div>
        <p style="font-family: 'Cormorant Garamond', Georgia, serif; font-style: italic; font-size: 1rem; color: #C9A227; margin-top: 1.25rem; margin-bottom: 0;">Equip Youth. Proclaim Hope.</p>
      </div>
      ...
      <div style="display: flex; flex-direction: column; gap: 1.25rem;">
        ...
        <div style="display: flex; gap: 1rem; align-items: flex-start;">
          <div style="width: 44px; height: 44px; background: linear-gradient(135deg, #D4AF37, #C9A227); border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span style="color: #FFFFFF; font-size: 1.2rem; font-weight: 700;">✝</span>
          </div>
          <div>
            <strong style="font-family: 'Inter', sans-serif; font-size: 0.95rem; color: #2B2B2B; display: block; margin-bottom: 0.15rem;">The Cross</strong>
            <p style="font-family: 'Inter', sans-serif; font-size: 0.9rem; color: #5A5955; line-height: 1.6; margin: 0;">Faith, sacrifice, and divine guidance — leadership rooted in spiritual principles. The central anchor of the emblem.</p>
          </div>
        </div>

        <div style="display: flex; gap: 1rem; align-items: flex-start;">
          <div style="width: 44px; height: 44px; background: linear-gradient(135deg, #38BDF8, #3A7CA5); border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span style="color: #FFFFFF; font-size: 1.2rem;">&#x2764;</span>
          </div>
          <div>
            <strong style="font-family: 'Inter', sans-serif; font-size: 0.95rem; color: #2B2B2B; display: block; margin-bottom: 0.15rem;">The Heart</strong>
            <p style="font-family: 'Inter', sans-serif; font-size: 0.9rem; color: #5A5955; line-height: 1.6; margin: 0;">Compassion, care, and outreach — wrapping the cross to symbolize faith integrated with active, embodied love.</p>
          </div>
        </div>

        <div style="display: flex; gap: 1rem; align-items: flex-start;">
          <div style="width: 44px; height: 44px; background: #0B1F3B; border-radius: 10px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
            <span style="color: #FFFFFF; font-size: 1.1rem;">&#x25CB;</span>
          </div>
          <div>
            <strong style="font-family: 'Inter', sans-serif; font-size: 0.95rem; color: #2B2B2B; display: block; margin-bottom: 0.15rem;">The Circle</strong>
            <p style="font-family: 'Inter', sans-serif; font-size: 0.9rem; color: #5A5955; line-height: 1.6; margin: 0;">Stability, trust, and institutional strength — unity and wholeness grounding the identity in credibility.</p>
          </div>
        </div>
